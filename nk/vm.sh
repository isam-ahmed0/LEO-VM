...
# =============================
# Enhanced Multi-VM Manager (Non-KVM)
# =============================
...
░▒▓████████▓▒░▒▓████████▓▒░▒▓██████▓▒░  
            ...
            POWERED BY HOPINGBOYZ (NON-KVM EDITION)
========================================================================
EOF
    echo
}
...
create_new_vm() {
    print_status "INFO" "Creating a new VM (Non-KVM Mode)"
...
start_vm() {
    ...
    if load_vm_config "$vm_name"; then
        print_status "INFO" "Starting VM: $vm_name (Software Emulation)"
        ...
        # Base QEMU command (NON-KVM VERSION)
        # Changed: -accel tcg instead of -enable-kvm
        # Changed: -cpu max instead of -cpu host
        local qemu_cmd=(
            qemu-system-x86_64
            -accel tcg
            -m "$MEMORY"
        ...
        if [[ "$GUI_MODE" == true ]]; then
            qemu_cmd+=(-vga virtio -display gtk,gl=off) # GL off often better for software rendering
        else
            qemu_cmd+=(-nographic -serial mon:stdio)
        fi

        # Add performance enhancements (software based)
        qemu_cmd+=(
        ...
        print_status "INFO" "Starting QEMU (TCG)..."
        "${qemu_cmd[@]}"
...
connect_vm_ssh() {
    ...
    if load_vm_config "$vm_name"; then
        if is_vm_running "$vm_name"; then
            print_status "INFO" "Preparing to connect to $vm_name..."
            echo "=========================================="
            ...
            echo "=========================================="
            print_status "INFO" "Launching SSH session. You may need to type 'yes' to accept the key fingerprint."
            print_status "INPUT" "Press Enter to connect..."
            read
            
            # Attempt SSH connection
            # -o UserKnownHostsFile=/dev/null to avoid conflicts with re-created VMs on same port
            ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p "$SSH_PORT" "$USERNAME@localhost" || true
            
            echo
            print_status "INFO" "SSH session ended."
        else
            print_status "ERROR" "VM $vm_name is not running. Please start it first (Menu Option 2)."
        fi
    fi
}
    ...
    while true; do
        ...
        case $choice in
            ...
            8)
                ...
                ;;
            9)
                print_status "INFO" "Initializing AI Assistant..."
                bash <(curl -s https://raw.githubusercontent.com/isam-ahmed0/vms/refs/heads/main/ai.sh)
                ;;
            10)
                if [ $vm_count -gt 0 ]; then
