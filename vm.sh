...
show_vm_info() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        echo "CPUs: $CPUS"
        echo "Disk: $DISK_SIZE"
        echo "Status: $(if is_vm_running "$vm_name"; then echo "RUNNING"; else echo "STOPPED"; fi)"
        echo "=========================================="
        echo
    fi
}
...
is_vm_running() {
    local vm_name=$1
    if pgrep -f "qemu-system-x86_64.*$vm_name" >/dev/null; then return 0; else return 1; fi
}
...
edit_vm_config() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        while true; do
            ...
            echo "  5) GUI Mode  6) Port Fwd  7) RAM       8) CPUs      9) Disk  0) Back"
            read -p "$(print_status "INPUT" "Choice: ")" edit_choice
            case $edit_choice in
                1) read -p "New Hostname: " HOSTNAME ;;
                2) read -p "New Username: " USERNAME ;;
                3) read -s -p "New Password: " PASSWORD; echo ;;
                4) read -p "New SSH Port: " SSH_PORT ;;
                5) read -p "GUI (y/n): " g; [[ $g =~ [Yy] ]] && GUI_MODE=true || GUI_MODE=false ;;
                6) read -p "Port Forwards: " PORT_FORWARDS ;;
                7) read -p "RAM (MB): " MEMORY ;;
                8) read -p "CPUs: " CPUS ;;
                9) read -p "Disk Size: " DISK_SIZE ;;
                0) break ;;
            esac
            if [[ "$edit_choice" -le 3 ]]; then setup_vm_image; fi
            save_vm_config
            read -p "Continue editing? (y/N): " c; [[ ! "$c" =~ [Yy] ]] && break
        done
    fi
}
...
resize_vm_disk() {
    local vm_name=$1
    if load_vm_config "$vm_name"; then
        read -p "New disk size (e.g., 50G): " s
        qemu-img resize "$IMG_FILE" "$s"
        DISK_SIZE="$s"
        save_vm_config
    fi
}
...
show_vm_performance() {
    ...
    if load_vm_config "$vm_name"; then
        if is_vm_running "$vm_name"; then
            local pid=$(pgrep -f "qemu-system-x86_64.*$IMG_FILE")
...
main_menu() {
    while true; do
        ...
        echo
        echo "1) Create  2) Start  3) Stop  4) Info  5) Edit  6) Delete  7) Resize  8) Perf  0) Exit"
        read -p "Choice: " c
        case $c in
            1) create_new_vm ;;
            2) [[ $vm_count -gt 0 ]] && { read -p "VM #: " n; start_vm "${vms[$((n-1))]}"; } ;;
            3) [[ $vm_count -gt 0 ]] && { read -p "VM #: " n; stop_vm "${vms[$((n-1))]}"; } ;;
            4) [[ $vm_count -gt 0 ]] && { read -p "VM #: " n; show_vm_info "${vms[$((n-1))]}"; } ;;
            5) [[ $vm_count -gt 0 ]] && { read -p "VM #: " n; edit_vm_config "${vms[$((n-1))]}"; } ;;
            6) [[ $vm_count -gt 0 ]] && { read -p "VM #: " n; delete_vm "${vms[$((n-1))]}"; } ;;
            7) [[ $vm_count -gt 0 ]] && { read -p "VM #: " n; resize_vm_disk "${vms[$((n-1))]}"; } ;;
            8) [[ $vm_count -gt 0 ]] && { read -p "VM #: " n; show_vm_performance "${vms[$((n-1))]}"; } ;;
            0) exit 0 ;;
        esac
        read -p "Press Enter..."
    done
...
# CHECK: Only run menu if executing directly, not if sourcing
